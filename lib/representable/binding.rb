  ...
  class Binding
    ...
    module EvaluateOption
      def evaluate_option(name, input, options)
        ...
        proc.(send(:exec_context, options), **options, user_options: options[:options][:user_options], input: input) # from Uber::Options::Value. # NOTE: this can also be the Proc object if it's not wrapped by Uber:::Value.
      end
    end
